...
SMLK_BOOL SMLK_LOAN_EMS_YUCHAI::Send0F3D(SMLK_BOOL need_handshake)
{
...
SMLK_BOOL SMLK_LOAN_EMS_YUCHAI::DoLoanActive(IN LoanCmdMsg &queue, OUT SMLK_UINT8 &result, std::vector<SMLK_UINT8> &i_gpsid, std::vector<SMLK_UINT8> &i_key)
{
    SMLK_LOGD("*******enter in func SMLK_LOAN_EMS_YUCHAI::(%s)*******",__func__);
    ...
    SysProperty::GetInstance()->SetValue(SYS_PRO_NAME_LCKCAR_TYPE_JTT808, to_string(JTT808_LCKCAR_TYPE_TORQUE));
...
SMLK_BOOL SMLK_LOAN_EMS_YUCHAI::CheckIgnonTime()
{
    return SMLK_TRUE;
}

SMLK_BOOL SMLK_LOAN_EMS_YUCHAI::ResultEncode(IN LoanCtrlResult &result)
{
    SMLK_LOGD("enter in func(SMLK_LOAN_EMS_YUCHAI::%s)\n", __func__);
...
void SMLK_LOAN_EMS_YUCHAI::EncodeMcuReport(SMLK_UINT8 result)
{
    SMLK_LOGD("*******SMLK_LOAN_EMS_YUCHAI::(%s)****result (%d)***",__func__, result);
    std::vector<SMLK_UINT8> output_vec;
    Msg0f40Resp resp;
    bzero(&resp, sizeof(Msg0f40Resp));
    resp.seq_id = 0xffff;
